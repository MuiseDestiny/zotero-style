...
export default class Views {
  ...
  public async registerCommands() {
    ...
    ztoolkit.Prompt.register([{
      ...
      callback: async (prompt) => {
        if (!(prompt.inputNode.value.endsWith(" ") && prompt.inputNode.value.length > 3)) {
          return prompt.showTip("You are currently in GPT mode. Please enter and end with a space to confirm.")
        }
        const secretKey = Zotero.Prefs.get(`${config.addonRef}.GPT.secretKey`)
        if (!secretKey) {
          return prompt.showTip("Your GPT secret key is null.")
        }
        prompt.showTip("GPT is being connected...")
